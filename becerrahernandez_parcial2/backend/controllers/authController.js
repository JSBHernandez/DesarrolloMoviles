...
function generateShortAccessToken(user) {
    return jwt.sign(user, process.env.ACCESS_TOKEN_SECRET, { expiresIn: process.env.SHORT_ACCESS_TOKEN_EXPIRATION }); // generacion token login
}
...
function validateToken(req, res, next) {   // Middleware para validar el token de acceso 11 - 21
...
async function login(req, res) {
    ...
    try {
        ...
        if (!user) {
            ...
            const accessToken = generateShortAccessToken(user); //token sesion
